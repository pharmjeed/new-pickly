...
read -r PK
[ -n "$PK" ] || die "لم تُدخل المفتاح القابل للنشر"
case "$PK" in pk_*) ;; *) die "المفتاح القابل للنشر يجب أن يبدأ بـpk_" ;; esac

printf 'المفتاح السري (sk_... — لن يظهر): '
read -rs SK
echo
[ -n "$SK" ] || die "لم تُدخل المفتاح السري"
case "$SK" in sk_*) ;; *) die "المفتاح السري يجب أن يبدأ بـsk_" ;; esac
