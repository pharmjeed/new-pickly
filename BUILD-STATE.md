...
## اعتماد Google Pay وحذف Apple Pay نهائياً (قرار المالك 2026-08-02)

- **Google Pay معتمدة افتراضياً**: `supportedMethods()` في ميسر تعيدها ما لم يُضبط
  `MOYASAR_GOOGLE_PAY=false` صراحة (كان العلم opt-in). compose الإنتاج افتراضيه true.
- **Apple Pay حُذفت من كل الطبقات**: صفحة الدفع (الجلسة والزر الأسود والعلامة وapplepay.d.ts) ·
  محوّل ميسر (مصدر applepay والعلم) · افتراضيات وقائمة أدمن `payments.methods`
  (المفتاح خارج enum الحفظ، وتُرشّح من الصفوف المخزنة تاريخياً في `paymentMethodsConfig`) ·
  تطبيق Expo القديم. عقود `packages/contracts` لم تُمسّ (حقول آبل فيها بقايا خاملة —
  تغيير العقد PR مستقل بمراجعة)، والخادم يرفض أي intent بها لأنها خارج القائمتين.
...
- Apple Pay ~~مخفي تلقائياً حتى `MOYASAR_APPLE_PAY=true`~~ — حُذفت نهائياً 2026-08-02 (انظر أعلى الملف).
