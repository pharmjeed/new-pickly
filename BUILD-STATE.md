...
إعادة تسعير. أُضيف أيضاً سهم «←» يتحرك باتجاه الإتمام، وارتفاع بظل عند التمرير
وانضغاط مرن عند اللمس (جوال: prop اختياري `arrow` في `LimeButton` + scale عند الضغط).
...
إرسال الطلب **بلا دفع** → MERCHANT_PENDING فوراً (عداد BR-1) → قبول الفرع بوقت تجهيز
(10/15/20/25 د) → موافقة العميل على الوقت خلال **5 د** (`confirm-prep-time` — يرفضه؟ إلغاء بلا رسوم)
→ الدفع خلال **5 د** (intent لا يُنشأ قبل الموافقة) → webhook النجاح = **لحظة الصفر**:
`PAYMENT_AUTHORIZED → PREPARING` آلياً + Capture فوري + `preparing_at = لحظة الدفع`
(منها عداد دقائق التجهيز) → «انطلقت الآن» مسموحة من PREPARING فقط.

- **لا استرجاع قبل الدفع**: رفض/timeout/انتهاء المهلتين كلها قبل قبض المال (حارس توافق خلفي
  للطلبات القديمة المدفوعة يبقى في reject/accept-timeout).
- **مؤقتا worker جديدان**: `prep_confirm_timeout` + `payment_timeout` (5 د، EXPIRED،
  إشعار order_expired_unconfirmed/order_expired_unpaid) — `apps/worker/src/handlers/post-accept-timeouts.ts`.
- **رادار الوصول** (docs/14§5-مكرر): `GET /v1/merchant/radar` + قائمة جانبية في board
  (أخضر/أصفر/أحمر/رمادي، تنبيه صوتي WebAudio + علامة حمراء عند ETA ≤ 1 د أو الوصول) ·
  عتبة `eta_threshold_1` جديدة في pickup (هجرة `20260711180000_eta_threshold_one`) —
  بلا أي استدعاء خرائط إضافي (تُشتق من لقطات ETA الموجودة).
- **لوحة الفرع**: تبويب «بانتظار الدفع» (MERCHANT_ACCEPTED/PAYMENT_PENDING/PAYMENT_FAILED،
  معلوماتي بعدادات المهل، منع بدء التحضير ORDER-4009) · KDS: الطلبات المدفوعة تدخل
  «قيد التحضير» آلياً.
- **واجهات العميل** (web + Expo): checkout بلا دفع («أرسل الطلب — الدفع بعد القبول») ·
  track: بطاقة موافقة الوقت (+عداد) ← بطاقة «ادفع الآن» (+عداد) ← «انطلقت الآن» بعد الدفع.
- **BR-4** صار بعد الدفع فقط (أثناء التحضير) · المجدول: الإرسال يؤكد الحجز والدفع بعد دخول
  الفترة والقبول (أُلغيت scheduled_expire عند الإنشاء).
- **الاختبارات محدثة**: order-states (13 اختباراً خضراء) · slice-j1 · suites-mandatory ·
  phase2 · Playwright j1 · k6 — الـintegration تُتحقق على السيرفر (لا DB محلياً بقرار المالك).
